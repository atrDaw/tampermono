...
(function () {
  ...
  // --- Funciones de simulación y lógica (idénticas a las tuyas) ---
  ...
  // Comprobador de URL continuo (para webs SPA que no recargan)
  function chequearRutaYMostrarBoton() {
    let btn = document.getElementById(ID_BOTON);
    
    // Si el botón no existe aún en el DOM, lo creamos
    if (!btn) {
      crearBotonFlotante();
      btn = document.getElementById(ID_BOTON);
    }

    // Comprobamos si la URL actual es exactamente la de exámenes
    if (window.location.href === URL_PERMITIDA) {
      ...
      if (btn && btn.style.display !== "none") {
        btn.style.display = "none"; // Lo ocultamos silenciosamente
        console.log("Ocultando botón de exportación fuera de /exam");
      }
    }
  }

  // Comprobar la URL cada 500ms (método ultra-seguro y ligero)
